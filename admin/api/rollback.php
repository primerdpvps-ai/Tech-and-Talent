<?php
/**
 * TTS PMS - Rollback API
 * Reverts page to selected version from history
 */
...
try {
    ...
    if (!$pageId || !$version) {
        throw new Exception('Page ID and version required');
    }
    ...
    $historyEntry = $db->fetchOne("
        SELECT * FROM tts_cms_history 
    ...
    if ($currentLayout) {
        $db->insert('tts_cms_history', [
            ...
            'change_description' => "Backup before rollback to version {$version}",
            'is_auto_backup' => true
        ]);
    ...
    // Create new layout from rollback version
    $rollbackLayoutJson = $historyEntry['layout_backup'] ?: $historyEntry['content_data'];
    
    $newLayoutId = $db->insert('tts_page_layouts', [
        'page_id' => $pageId,
        'layout_json' => $rollbackLayoutJson,
    ...
    // Generate physical file with rollback content
    $layoutData = json_decode($rollbackLayoutJson, true);
    $filePath = null;
    ...
    log_admin_action(
        ...
        $layoutData,
        "Rolled back page '{$page['title']}' to version {$version}"
    );
    
    echo json_encode([
        'success' => true,
        'message' => "Page rolled back to version {$version}",
        'new_layout_id' => $newLayoutId,
        'file_path' => $filePath,
        'new_version' => ($currentLayout['version'] ?? 0) + 1
    ]);
